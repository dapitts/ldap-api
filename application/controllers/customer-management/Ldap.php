...
class Ldap extends CI_Controller
{
	public function __construct()
	{
		...
		$this->utility->fedramp_section_access_exception();
	...
	public function index($method, $args = array())
	{
		...
		$data['ldap_info']          = $this->ldap_api->redis_info($asset['seed_name']);
		...
		if (!is_null($data['ldap_info']))
		{
			$data['action'] = 'modify';

			if (intval($data['ldap_info']['tested']))
			{
				$data['show_activation']    = TRUE;
				$data['api_tested']         = TRUE;

				if (intval($data['ldap_info']['request_sent']))
				{
					$data['request_was_sent'] = TRUE;
				}

				if (intval($data['ldap_info']['enabled']))
				{
		...
		$this->load->view('customer-management/ldap/start', $data);
	...
	public function create()
	{
		...
		if ($this->input->method(TRUE) === 'POST')
		{
			...
			$this->form_validation->set_rules('port', 'Port', 'trim|required|greater_than_equal_to[1]|less_than_equal_to[65535]');
			...
			if ($this->form_validation->run()) 
			{
				...
				if ($this->ldap_api->create_ldap_redis_key($asset['seed_name'], $redis_data))
				{
					# Write To Logs
					$log_message = '[LDAP API Created] user: '.$this->session->userdata('username').' | for client: '.$asset['client'];
					$this->utility->write_log_entry('info', $log_message);

					# Success
					$this->session->set_userdata('my_flash_message_type', 'success');
					$this->session->set_userdata('my_flash_message', '<p>LDAP API settings were successfully created.</p>');

					redirect('/customer-management/ldap/'.$asset['code']);
				}
				else
				{
		...
		# Page Views
		$this->load->view('assets/header');
		$this->load->view('customer-management/ldap/create', $data);
		$this->load->view('assets/footer');
	}
	...
	public function modify()
	{
		...
		if ($this->input->method(TRUE) === 'POST')
		{
			...
			if ($this->form_validation->run())
			{
				...
				{
					# Write To Logs
					$log_message = '[LDAP API Modified] user: '.$this->session->userdata('username').' | for client: '.$asset['client'];
					$this->utility->write_log_entry('info', $log_message);

					# Success
					$this->session->set_userdata('my_flash_message_type', 'success');
					$this->session->set_userdata('my_flash_message', '<p>LDAP API settings were successfully updated.</p>');
		...
		# Page Views
		$this->load->view('assets/header');
		$this->load->view('customer-management/ldap/modify', $data);
		$this->load->view('assets/footer');
	}
	...
	public function api_test()
	{
		...
		$response   = $this->ldap_api->fetch($asset['seed_name'], 'cn', 'A*');
		...
		if ($response['success'])
		{
			...
			foreach ($response['response']['results'] as $idx => $result)
			{
				...
				$results[] = array(
					...
					'title'         => $result['title'][0] ?? 'N/A',
					'mail'          => $result['mail'][0] ?? 'N/A',
					'department'    => $result['department'][0] ?? 'N/A',
				);
			}

			if ($result_count)
			{
				$this->ldap_api->redis_info($asset['seed_name'], NULL, 'SET', array('tested' => '1'));
			}
	...
	public function activate()
	{
		...
		$ldap_info                      = $this->ldap_api->redis_info($asset['seed_name']);
		$data['authorized_to_modify']   = $this->account->get_authorized_to_modify($asset['id']);
		$data['client_code']            = $asset['code'];
		$data['client_title']           = $asset['client'];
		$data['requested']              = $ldap_info['request_sent'];
		$data['request_user']           = $ldap_info['request_user'] ?? NULL;
		$data['terms_agreed']           = intval($ldap_info['terms_agreed']);

		$this->load->view('customer-management/ldap/activate', $data);
	}
	...
	public function do_activate()
	{
		...
		if ($this->form_validation->run()) 
		{
			...
			if ($this->ldap_api->change_api_activation_status($asset['seed_name'], $requested_by, TRUE))
			{
				$this->account->send_api_activation_notification($asset['id'], 'ldap', $requested_name);

				# Write To Logs
				$log_message = '[LDAP API Enabled] user: '.$this->session->userdata('username').', has enabled api for customer: '.$asset['client'].', per the request of '.$requested_name;
				$this->utility->write_log_entry('info', $log_message);

				# Set Success Alert Response
				$this->session->set_userdata('my_flash_message_type', 'success');
				$this->session->set_userdata('my_flash_message', '<p>The LDAP API for: <strong>'.$asset['client'].'</strong>, has been successfully enabled.</p>');

				$response = array(
					'success'   => true,
					'goto_url'  => '/customer-management/ldap/'.$asset['code']
				);
	...
	public function disable()
	{
		...
		$this->load->view('customer-management/ldap/disable', $data);
	}
	...
	public function do_disable()
	{
		...
		if ($this->form_validation->run()) 
		{
			...
			if ($this->ldap_api->change_api_activation_status($asset['seed_name'], $requested_by, FALSE))
			{
				$this->account->send_api_disabled_notification($asset['id'], 'ldap', $requested_name);

				# Write To Logs
				$log_message = '[LDAP API Disabled] user: '.$this->session->userdata('username').', has disabled api for customer: '.$asset['client'].', per the request of '.$requested_name;
				$this->utility->write_log_entry('info', $log_message);

				# Set Success Alert Response
				$this->session->set_userdata('my_flash_message_type', 'success');
				$this->session->set_userdata('my_flash_message', '<p>The LDAP API for: <strong>'.$asset['client'].'</strong>, has been successfully disabled.</p>');
